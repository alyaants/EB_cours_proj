...
function addToCart(event) {
  const productElement = event.target.closest(".catalog-catalog__item");
  if (productElement) {
      const productName = productElement.querySelector(".prodTitle").textContent;
      const firm = productElement.querySelector(".firm").textContent;
      const productPrice = productElement.querySelector("span").textContent;
      const productImg = productElement.querySelector("img").src; // Получаем URL изображения

      const productId = Date.now(); // Уникальный идентификатор товара

      const product = {
          id: productId, // Добавляем идентификатор
          name: productName,
          firm: firm,
          price: productPrice,
